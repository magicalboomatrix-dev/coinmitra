    ...
    async function fetchRecords() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            const formatted = data.swaps.map((tx: any) => ({
              id: tx.id,
              type: tx.type,
              ...
              inrAmount: `₹${tx.inrAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
              rate: tx.rate,
              status: tx.status
            }));
        ...
        console.error('Failed to load swap records:', err);
      } finally {
        setIsLoading(false);
      }
    }
    ...
    <div className="min-h-screen bg-[#f7f7f7] dark:bg-[#0a0b0d] text-zinc-800 dark:text-[#fafafa] flex flex-col transition-colors duration-300 pb-24">
      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-4 flex flex-col">
        ...
        <div className="flex justify-between items-center w-full mb-5">
          {/* Blue Back Arrow pointing to /swap */}
          <Link
            href="/swap"
            ...
            records.map((record) => (
              ...
              >
                <div className="flex items-center space-x-3.5">
                  {/* Left Swap Icon */}
                  <div className="w-10 h-10 rounded-xl bg-orange-500/10 dark:bg-orange-500/15 flex items-center justify-center text-orange-500 flex-shrink-0">
                    <ArrowUpDown className="w-5 h-5 stroke-[2.5]" />
                  </div>
                  
                  {/* Details */}
                  <div>
                    <h4 className="font-bold text-[14px] text-zinc-800 dark:text-white leading-tight">
                      {record.amount} &rarr; {record.inrAmount}
                    </h4>
                    <p className="text-[11px] text-zinc-400 dark:text-zinc-500 font-medium mt-1">
                      Rate: ₹{record.rate.toFixed(2)} &middot; {record.dateTime}
                    </p>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-[11px] font-semibold text-emerald-500 dark:text-emerald-400 inline-flex items-center select-none">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 mr-1 inline-block" />
                    {record.status}
                  </span>
                </div>
              </div>
