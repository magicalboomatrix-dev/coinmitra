    ...
    async function fetchRecords() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            const formatted = data.withdrawals.map((tx: any) => ({
              id: tx.id,
              type: tx.type,
    ...
    <div className="min-h-screen bg-[#f7f7f7] dark:bg-[#0a0b0d] text-zinc-800 dark:text-[#fafafa] flex flex-col transition-colors duration-300 pb-24">
      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-4 flex flex-col">
        ...
        <div className="flex justify-between items-center w-full mb-5">
          {/* Blue Back Arrow pointing to /withdraw */}
          <Link
            href="/withdraw"
          ...
          {isLoading ? (
            ...
            records.map((record) => {
              ...
              return (
                <div
                  key={record.id}
                  className="bg-white dark:bg-[#121417] border border-black/[0.03] dark:border-white/[0.03] rounded-[20px] p-4 flex flex-col shadow-[0_4px_12px_rgba(0,0,0,0.015)] text-left"
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center space-x-3.5">
                      {/* Left Upward Icon */}
                      <div className="w-10 h-10 rounded-xl bg-red-500/10 dark:bg-[#ff8787]/15 flex items-center justify-center text-red-500 flex-shrink-0">
                    ...
                    <div className="text-right">
                      <span className="text-[15px] font-bold text-rose-500 dark:text-rose-400 tracking-wide block leading-none font-display">
                        {record.amount}
                      </span>
                  ...
                  {record.remarks && (
                    <div className="mt-2.5 pt-2 border-t border-black/[0.03] dark:border-white/[0.03] text-[10px] font-semibold text-zinc-400 dark:text-zinc-500">
