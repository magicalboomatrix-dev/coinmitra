    ...
    async function fetchRecords() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.success) {
            const formatted = data.records.map((tx: any) => ({
              id: tx.id,
              type: tx.type,
    ...
    <div className="min-h-screen bg-[#f7f7f7] dark:bg-[#0a0b0d] text-zinc-800 dark:text-[#fafafa] flex flex-col transition-colors duration-300 pb-24">
        ...
        <div className="space-y-3 flex-1 overflow-y-auto no-scrollbar">
          {isLoading ? (
            ...
            records.map((record) => {
              ...
              let iconColorClass = 'text-[#007aff]';
              let iconBgClass = 'bg-blue-500/10 dark:bg-[#007aff]/15';
              let amountColorClass = 'text-zinc-800 dark:text-white';

              if (record.type === 'Deposit') {
                IconComponent = ArrowDown;
                iconColorClass = 'text-emerald-500 dark:text-emerald-400';
                iconBgClass = 'bg-emerald-500/10 dark:bg-[#10b981]/15';
                amountColorClass = 'text-emerald-500 dark:text-emerald-400';
              } else if (record.type === 'Withdraw') {
                IconComponent = ArrowUp;
                iconColorClass = 'text-rose-500 dark:text-rose-400';
                iconBgClass = 'bg-rose-500/10 dark:bg-[#f43f5e]/15';
                amountColorClass = 'text-rose-500 dark:text-rose-400';
              } else if (record.type === 'Swap') {
                IconComponent = ArrowUpDown;
                iconColorClass = 'text-amber-500 dark:text-amber-400';
                iconBgClass = 'bg-amber-500/10 dark:bg-[#f59e0b]/15';
                amountColorClass = 'text-amber-500 dark:text-amber-400';
              }

              return (
                <div
                  key={record.id}
                  className="bg-white dark:bg-[#121417] border border-black/[0.03] dark:border-white/[0.03] rounded-[20px] p-4 flex flex-col shadow-[0_4px_12px_rgba(0,0,0,0.015)]"
                >
                  <div className="flex items-center justify-between w-full">
                    ...
                    <div className="text-right">
                      <span className={`text-[15px] font-bold ${amountColorClass} tracking-wide block leading-none font-display`}>
                  ...
                  {record.remarks && (
                    <div className="mt-2.5 pt-2 border-t border-black/[0.03] dark:border-white/[0.03] text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 text-left">
      ...
      <div className="fixed bottom-4 inset-x-0 flex justify-center z-50 px-4">
        <div className="w-full max-w-md bg-white/80 dark:bg-[#121417]/80 backdrop-blur-lg border border-black/[0.04] dark:border-white/10 rounded-full px-3 py-2 flex justify-between items-center shadow-lg">
          {/* Home Tab (Active) */}
          <Link href="/dashboard" className="flex flex-col items-center justify-center flex-1 py-1 text-[#007aff] cursor-pointer">
